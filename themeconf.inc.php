...
Author URI: https://github.com/kitsguru/piwigo-bootstrap-darkroom
*/
require_once(PHPWG_THEMES_PATH . 'bootstrap_darkroom_home/include/themecontroller.php');
require_once(PHPWG_THEMES_PATH . 'bootstrap_darkroom_home/include/config.php');

$themeconf = array(
    'name' => 'bootstrap_darkroom_home',
...
function strip_breadcrumbs() {
  ...
  if (!empty($title)) {
    ...
    $home_link_content = '<a href="' . $u_home . '" title="' . $home_link_orig->nodeValue . '"><i class="fa fa-paint-brush" aria-hidden="true"></i></a >';
    if ($nr_links == 1) {
...
function replace_home_link($content, &$smarty) {
  ...
  $replace = '<div class="navbar-brand"><a href="/" title="Main Site"><i class="fa fa-home" aria-hidden="true"></i></a> {$LEVEL_SEPARATOR} <a href="{$U_HOME}" title="{"Home"|@translate}">
